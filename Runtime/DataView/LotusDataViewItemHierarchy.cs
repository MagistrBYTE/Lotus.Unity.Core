...
//---------------------------------------------------------------------------------------------------------------------
/** \file LotusPatternViewItemHierarchy.cs
...
namespace Lotus
{
	namespace Core
	{
		...
		public interface ILotusViewItemHierarchy : ILotusViewItem, ILotusOwnerObject
		{
			...
			/// <param name="exclude">Исключаемый элемент отображения</param>
			//---------------------------------------------------------------------------------------------------------
			void UnsetAllPresent(ILotusViewItemHierarchy exclude);
			...
			where TData : class
		{
			...
			#region ======================================= СВОЙСТВА ILotusGroupHierarchy =============================
			/// <summary>
			...
			//---------------------------------------------------------------------------------------------------------
			public override void AttachOwnedObject(ILotusOwnedObject owned_object)
			{
				//base.AttachOwnedObject(owned_object);
			}
			...
			/// <param name="owned_object">Объект</param>
			//---------------------------------------------------------------------------------------------------------
			public override void DetachOwnedObject(ILotusOwnedObject owned_object)
			{
				//base.DetachOwnedObject(owned_object);
			}
			...
			/// <param name="exclude">Исключаемый элемент отображения</param>
			//---------------------------------------------------------------------------------------------------------
			public virtual void UnsetAllPresent(ILotusViewItemHierarchy exclude)
			{
				if (exclude != null)
				{
					for (Int32 i = 0; i < mCount; i++)
					{
						if (Object.ReferenceEquals(mArrayOfItems[i], exclude) == false)
						{
							...
							mArrayOfItems[i].UnsetAllPresent(exclude);
						}
					}
				}
				else
			...
			#region ======================================= МЕТОДЫ ILotusGroupHierarchy ===============================
			//---------------------------------------------------------------------------------------------------------
			/// <summary>
