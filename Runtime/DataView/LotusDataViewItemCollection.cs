...
//---------------------------------------------------------------------------------------------------------------------
/** \file LotusDataViewItemCollection.cs
...
namespace Lotus
{
	namespace Core
	{
		...
		public interface ILotusCollectionView : ILotusOwnerObject
		{
			...
			/// <param name="exclude">Исключаемый элемент отображения</param>
			//---------------------------------------------------------------------------------------------------------
			void UnsetAllPresent(ILotusViewItem exclude);
			#endregion
		}
			...
			where TData : class
		{
			...
			/// <param name="exclude">Исключаемый элемент отображения</param>
			//---------------------------------------------------------------------------------------------------------
			public virtual void UnsetAllPresent(ILotusViewItem exclude)
			{
				if (exclude != null)
				{
					for (Int32 i = 0; i < mCount; i++)
			...
			protected virtual void OnCollectionChangedHandler(Object sender, NotifyCollectionChangedEventArgs args)
			{
				switch (args.Action)
				{
					case NotifyCollectionChangedAction.Add:
						{
							TData data = (TData)args.NewItems[0];
