...
//---------------------------------------------------------------------------------------------------------------------
/** \file LotusDataViewItemHierarchyCollection.cs
...
namespace Lotus
{
	namespace Core
	{
			...
			where TData : class
		{
			...
			/// <param name="exclude">Исключаемый элемент отображения</param>
			//---------------------------------------------------------------------------------------------------------
			public override void UnsetAllPresent(ILotusViewItem exclude)
			{
				if (exclude != null)
				{
					for (Int32 i = 0; i < mCount; i++)
					{
						if (Object.ReferenceEquals(mArrayOfItems[i], exclude) == false)
						{
							...
							mArrayOfItems[i].UnsetAllPresent(exclude as ILotusViewItemHierarchy);
						}
					}
